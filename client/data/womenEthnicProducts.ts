...
const generateWomenEthnicProducts = (): Product[] => {
  ...
  for (let i = 1; i <= 500; i++) {
    ...
    let basePrice = 500;
    if (productType.includes("Lehenga") || productType.includes("Wedding"))
      basePrice = 3000;
    else if (productType.includes("Saree") || productType.includes("Suit"))
      basePrice = 1200;
    else if (productType.includes("Kurti")) basePrice = 600;
    else if (productType.includes("Gown") || productType.includes("Anarkali"))
      basePrice = 1500;
